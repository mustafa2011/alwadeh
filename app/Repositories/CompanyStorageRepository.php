...
class CompanyStorageRepository
{
    ...
    public function createCompany(array $data): bool
    {
        ...
        try {
            ...
            try {
                $this->createItemTaxCategories($companyId);
            } catch (Throwable $e) {
                throw new Exception("ITEM TAX CATEGORIES: ".$e->getMessage());
            }
            try {
                $this->createItemUnits($companyId);
            } catch (Throwable $e) {
                throw new Exception("ITEM UNITS: ".$e->getMessage());
            }
            try {
                $this->createItemCategories($companyId);
            } catch (Throwable $e) {
    ...
    private function createItemCategories(int $companyId): void
    {
        $stmt = Database::getConnection()->prepare(
            ...
            VALUES
            (?, 'GEN', 'General', 'General category')"
        );
        $stmt->execute([$companyId]);
    }
    private function createItemTaxCategories(int $companyId): void
    {
        $stmt = Database::getConnection()->prepare("
            INSERT INTO item_tax_categories
            (company_id, tax_category_id, tax_scheme_id, tax_percent, description)
            VALUES
            (?, 'S', 'VAT', 15.00, 'Standard VAT 15%'),
            (?, 'Z', 'VAT', 0.00, 'Zero rated VAT'),
            (?, 'E', 'VAT', 0.00, 'Exempt VAT'),
            (?, 'O', 'VAT', 0.00, 'Outside scope VAT')            
        ");
        $stmt->execute([$companyId, $companyId, $companyId, $companyId]);
    }
    private function createItemUnits(int $companyId): void
    {
        $stmt = Database::getConnection()->prepare("
            INSERT INTO item_units
            (company_id, unit_code, unit_name, ubl_unit_code)
            VALUES
            (?, 'EA', 'Each', 'EA'),
            (?, 'KGM', 'Kilogram', 'KGM'),
            (?, 'LTR', 'Litre', 'LTR'),
